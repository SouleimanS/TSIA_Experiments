...
class VideoLLAMA(Blip2Base):
    ...
    def from_config(cls, cfg):
        ...
        # PEFT-wrap LLM AFTER raw-LLM ckpts (1,2) but BEFORE LoRA ckpt (3)
        from peft import LoraConfig, get_peft_model
        _lora_cfg = LoraConfig(r=16, lora_alpha=16, lora_dropout=0.0, target_modules=["q_proj","k_proj","v_proj","o_proj"], bias="none", task_type="CAUSAL_LM")
        model.llama_model = get_peft_model(model.llama_model, _lora_cfg)
        for _n, _p in model.llama_model.named_parameters():
            _p.requires_grad = False
        print("Wrapped LLaMA with PEFT LoRA before loading ckpt_3")
