#!/bin/bash
#PBS -N v6_stdvib_smoke
#PBS -P gae50891
#PBS -q rt_HF
#PBS -l select=1
#PBS -l walltime=00:30:00
#PBS -j oe
#PBS -o v6_stdvib_smoke.qsub.log
set -euo pipefail
cd "$PBS_O_WORKDIR"
...
mkdir -p runs/v6_stdvib_smoke
echo "=== SMOKE: variant b, video_vib=standard, 10 steps ==="
python -u -m av_ib.train.train_v6 \
    ...
    --variant b \
    --video-vib standard \
    --log-path runs/v6_stdvib_smoke/log.jsonl \
    --ckpt-path runs/v6_stdvib_smoke/final.pt \
    --print-every 1 \
    --save-every 10
echo "=== Done $(date) ==="
