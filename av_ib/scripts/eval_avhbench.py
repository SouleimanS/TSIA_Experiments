"""Evaluate v1/v2/v3 checkpoints on the AVHBench test set.

Loads runs/avqa_{variant}_3ep/best.pt and runs forward_generate on the
5,302 yes/no items of AVHBench (filtering out the 1,106 captioning items).

Outputs (under results/):
    avhbench_{variant}.json  - per-task and overall metrics
    avhbench_{variant}.csv   - per-item predictions for inspection

Usage:
    python scripts/eval_avhbench.py --variant v1
"""
from __future__ import annotations
import argparse
import csv
import json
import re
import sys
import time
from collections import Counter, defaultdict
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parent.parent))

import torch
from av_ib.eval.avhbench import _load_video, _load_audio, _build_prompt


HOME = Path.home()
AVHBENCH_ROOT = HOME / "SOULEIMAN_repo" / "datasets" / "AVHBench" / "data" / "AVHBench_v0"
AVHBENCH_QA = AVHBENCH_ROOT / "json" / "qa.json"
AVHBENCH_VIDEOS = AVHBENCH_ROOT / "video"
RESULTS_DIR = Path("results")

JUDGMENT_TASKS = {
    "Audio-driven Video Hallucination",
    "Video-driven Audio Hallucination",
    "AV Matching",
}


def parse_yes_no(text: str) -> str:
    head = text.strip().lower()[:30]
    if re.search(r"\byes\b", head):
        return "Yes"
    if re.search(r"\bno\b", head):
        return "No"
    return "??"


def compute_task_metrics(records: list) -> dict:
    n = len(records)
    if n == 0:
        return {"n": 0}
    tp = sum(1 for r in records if r["label"] == "Yes" and r["pred"] == "Yes")
    tn = sum(1 for r in records if r["label"] == "No" and r["pred"] == "No")
    fp = sum(1 for r in records if r["label"] == "No" and r["pred"] == "Yes")
    fn = sum(1 for r in records if r["label"] == "Yes" and r["pred"] == "No")
    qq = sum(1 for r in records if r["pred"] == "??")
    acc = (tp + tn) / n
    prec = tp / (tp + fp) if (tp + fp) > 0 else 0.0
    rec = tp / (tp + fn) if (tp + fn) > 0 else 0.0
    f1 = 2 * prec * rec / (prec + rec) if (prec + rec) > 0 else 0.0
    yes_pct = (tp + fp) / n * 100
    return {
        "n": n, "acc": acc, "precision": prec, "recall": rec, "f1": f1,
        "yes_pct": yes_pct, "TP": tp, "TN": tn, "FP": fp, "FN": fn, "non_yes_no": qq,
    }


def build_model(variant: str, device: str):
    if variant == "v1":
        from av_ib.model.av_model_v1 import AVModelV1
        return AVModelV1(use_lora=True).to(device)
    if variant == "v2":
        from av_ib.model.av_model_v2 import AVModelV2
        return AVModelV2(use_lora=True).to(device)
    if variant == "v3":
        from av_ib.model.av_model_v3 import AVModelV3
        return AVModelV3(use_lora=True).to(device)
    raise ValueError(f"Unknown variant: {variant}")


def load_checkpoint(model, ckpt_path, device):
    print(f"Loading {ckpt_path}", flush=True)
    sd = torch.load(ckpt_path, map_location=device)
    own = dict(model.named_parameters())
    n_loaded = n_missing = 0
    for k, v in sd.items():
        if k in own:
            own[k].data.copy_(v.data)
            n_loaded += 1
        else:
            n_missing += 1
    print(f"  Loaded {n_loaded}/{len(sd)} params  (missing: {n_missing})", flush=True)


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--variant", required=True, choices=["v1", "v2", "v3"])
    ap.add_argument("--max-items", type=int, default=None)
    args = ap.parse_args()

    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    device = "cuda"

    with open(AVHBENCH_QA) as f:
        all_items = json.load(f)
    items = [d for d in all_items if d["task"] in JUDGMENT_TASKS]
    print(f"Loaded {len(items)} yes/no items (filtered from {len(all_items)} total)", flush=True)
    if args.max_items:
        items = items[: args.max_items]
        print(f"  Smoke test: limiting to {len(items)} items", flush=True)

    task_counts = Counter(d["task"] for d in items)
    label_counts = Counter(d["label"] for d in items)
    print(f"  Tasks: {dict(task_counts)}", flush=True)
    print(f"  Labels: {dict(label_counts)}", flush=True)

    print(f"\nBuilding model: {args.variant}", flush=True)
    model = build_model(args.variant, device)
    ckpt = Path(f"runs/avqa_{args.variant}_3ep/best.pt")
    load_checkpoint(model, ckpt, device)
    model.eval()
    s = model.trainable_summary()
    print(f"  Total trainable: {s['__total_trainable__']:,}", flush=True)

    print(f"\nRunning eval on {len(items)} items...", flush=True)
    predictions = []
    n_failed = 0
    t0 = time.time()
    for i, rec in enumerate(items):
        vid_path = AVHBENCH_VIDEOS / f"{rec['video_id']}.mp4"
        try:
            videos = _load_video(str(vid_path), device)
            audios = _load_audio(str(vid_path), device)
            prompt = _build_prompt(rec["task"], rec["text"])
            with torch.no_grad():
                out = model.forward_generate(videos, audios, [prompt], max_new_tokens=10)[0]
            pred = parse_yes_no(out)
            predictions.append({
                "video_id": rec["video_id"], "task": rec["task"], "text": rec["text"],
                "label": rec["label"], "pred": pred, "raw": out[:80],
            })
        except Exception as e:
            n_failed += 1
            predictions.append({
                "video_id": rec["video_id"], "task": rec["task"], "text": rec["text"],
                "label": rec["label"], "pred": "FAIL", "raw": str(e)[:80],
            })
        if (i + 1) % 100 == 0:
            elapsed = time.time() - t0
            rate = (i + 1) / elapsed
            eta = (len(items) - i - 1) / rate
            n_correct = sum(1 for p in predictions if p["pred"] == p["label"])
            print(f"  [{i+1}/{len(items)}]  acc-so-far={n_correct/(i+1):.4f}  "
                  f"failed={n_failed}  {rate:.1f} it/s  eta={eta/60:.0f}min", flush=True)

    total_time = time.time() - t0
    print(f"\nEval done in {total_time/60:.1f} min  failed={n_failed}", flush=True)

    by_task = defaultdict(list)
    for p in predictions:
        if p["pred"] != "FAIL":
            by_task[p["task"]].append(p)

    results = {"variant": args.variant, "n_items": len(items), "n_failed": n_failed,
               "eval_time_s": total_time, "per_task": {}, "overall": {}}

    print("\n=== Per-task metrics ===")
    for task in sorted(JUDGMENT_TASKS):
        recs = by_task[task]
        m = compute_task_metrics(recs)
        results["per_task"][task] = m
        print(f"  {task}:")
        print(f"    n={m['n']}  acc={m['acc']:.4f}  "
              f"P={m['precision']:.4f}  R={m['recall']:.4f}  F1={m['f1']:.4f}  "
              f"Yes%={m['yes_pct']:.1f}")

    all_recs = [p for p in predictions if p["pred"] != "FAIL"]
    m = compute_task_metrics(all_recs)
    results["overall"] = m
    print(f"\n  OVERALL:")
    print(f"    n={m['n']}  acc={m['acc']:.4f}  "
          f"P={m['precision']:.4f}  R={m['recall']:.4f}  F1={m['f1']:.4f}  "
          f"Yes%={m['yes_pct']:.1f}")

    out_json = RESULTS_DIR / f"avhbench_{args.variant}.json"
    with open(out_json, "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nSaved metrics to {out_json}", flush=True)

    out_csv = RESULTS_DIR / f"avhbench_{args.variant}.csv"
    with open(out_csv, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=["video_id", "task", "text", "label", "pred", "raw"])
        w.writeheader()
        w.writerows(predictions)
    print(f"Saved predictions to {out_csv}", flush=True)


if __name__ == "__main__":
    main()