"""v1 baseline on MUSIC-AVQA with noise-augmented training, clean eval."""
...
def main():
    ...
    print(f"=== Config: noise_mode={args.noise_mode}, noise_sigma={args.noise_sigma}, output_dir={OUT_DIR} ===", flush=True)
    ...
    print("=== Building AVModelV1 ===", flush=True)
    ...
    for epoch in range(NUM_EPOCHS):
        for batch in train_dl:
            ...
            if global_step % 10 == 0:
                print(f"  step {global_step:5d} (ep{epoch})  loss={rec['loss']:.4f}  gnorm={rec['grad_norm']:.2f}  elapsed={rec['elapsed_s']:.0f}s", flush=True)
            ...
            if (global_step + 1) % EVAL_EVERY == 0 or is_final:
                ...
                print(f"  acc={ev['accuracy']:.4f} ({ev['n_correct']}/{ev['n_total']})", flush=True)
