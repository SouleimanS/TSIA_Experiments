"""v4 C-MIB on MUSIC-AVQA with noise-augmented training, clean eval."""
...
def main():
    ...
    args = ap.parse_args()
    BETA_V = args.beta_v; BETA_A = args.beta_a; BETA_J = args.beta_j
    AUX_WEIGHT = args.aux_weight
    OUT_DIR = Path(args.output_dir)
    SMOKE_STEPS = args.smoke_steps
    print(f"=== Config: beta_v={BETA_V} beta_a={BETA_A} beta_j={BETA_J} aux={AUX_WEIGHT} noise={args.noise_mode}@{args.noise_sigma} out={OUT_DIR} ===", flush=True)
    ...
    print(f"=== Building AVModelV4 ===", flush=True)
    ...
    for epoch in range(NUM_EPOCHS):
        for batch in train_dl:
            ...
            nll, nll_av, nll_aa, kl_v, kl_a, kl_j = model.forward_train(
                videos, audio_mels, batch["prompts"], batch["answers"]
            )
            loss = nll + BETA_V*kl_v + BETA_A*kl_a + BETA_J*kl_j + AUX_WEIGHT*(nll_av + nll_aa)
            ...
            rec = {"step": global_step, "epoch": epoch, "loss": float(loss.item()),
                   "nll": float(nll.item()), "nll_aux_v": float(nll_av.item()), "nll_aux_a": float(nll_aa.item()),
                   "kl_v": float(kl_v.item()), "kl_a": float(kl_a.item()), "kl_j": float(kl_j.item()),
                   "grad_norm": float(gnorm), "elapsed_s": time.time() - t0}
            train_log.write(json.dumps(rec) + "\n"); train_log.flush()
            if global_step % 10 == 0:
                print(f"  step {global_step:5d} (ep{epoch})  loss={rec['loss']:.4f}  nll={rec['nll']:.3f} av={rec['nll_aux_v']:.3f} aa={rec['nll_aux_a']:.3f}  kl_v={rec['kl_v']:.3f} kl_a={rec['kl_a']:.3f} kl_j={rec['kl_j']:.3f}  gnorm={rec['grad_norm']:.2f}  el={rec['elapsed_s']:.0f}s", flush=True)
