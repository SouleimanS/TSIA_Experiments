"""3-epoch single-GPU training on AVQA yes/no for AVModelV3 (VIB only).

Identical to train_avqa_3ep.py but:
  - Instantiates AVModelV3 (joint VIB after concat)
  - forward_train returns (nll, kl); loss = nll + BETA * kl
  - Logs nll, kl, total_loss separately for diagnosis
  - Writes to runs/avqa_v3_3ep/
"""
...
def main():
    ...
    print("=== Building AVModelV3 (VIB only, beta={}) ===".format(BETA), flush=True)
    model = AVModelV3(use_lora=True).to(DEVICE)
    s = model.trainable_summary()
    print(f"  Trainable: {s['__total_trainable__']:,}", flush=True)
    print(f"  Bottleneck adds: {s.get('bottleneck', 0):,} params", flush=True)
    ...
    print(f"\n=== Training {NUM_EPOCHS} epochs, eval every {EVAL_EVERY}, beta={BETA} ===\n", flush=True)
    model.train()
    t0 = time.time()
    best_acc = 0.0
    global_step = 0
