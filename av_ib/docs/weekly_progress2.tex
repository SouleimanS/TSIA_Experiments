...
\begin{block}{Per-modality ablation (NLL vs.\ \texttt{identity})}
{\footnotesize
\begin{tabular}{l r}
\toprule
\textbf{Ablation} & \textbf{$\Delta$NLL} \\
\midrule
\texttt{identity} (reference) & --- \\
\rowcolor{paperD!12}
\texttt{video\_only} (drop audio) & $+4.4$\,\% \\
\texttt{audio\_only} (drop video) & TODO \\
\texttt{zero} (destroy both) & TODO \\
\texttt{mean} & TODO \\
\texttt{shuffle} & TODO \\
\bottomrule
\end{tabular}}
