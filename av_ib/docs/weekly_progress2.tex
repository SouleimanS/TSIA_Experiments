...
\end{tikzpicture}\\[2pt]
{\tiny Removing audio (+8.3\%) ≈ shuffling everything (+6.3\%). Only video matters.}
\end{column}

% ---- plot 3: training curves ----
\begin{column}{0.33\textwidth}
\centering
{\scriptsize\textbf{Training curves: video rate falls, audio frozen}}\\[3pt]
\begin{tikzpicture}[font=\tiny, >=stealth]
% x: 0..2000->0..4, y: 0..12000->0..3.2
\def\xsc{0.002}    % 4/2000
\def\ysc{0.0002667} % 3.2/12000
% grid
\foreach \yv in {3000,6000,9000,12000}{
  \pgfmathsetmacro{\yp}{\yv*\ysc}
  \draw[black!10] (0,\yp)--(4,\yp);
}
% axes
\draw[->](0,0)--(4.3,0) node[right]{step};
\draw[->](0,0)--(0,3.5) node[above]{KL};
% y ticks
\foreach \yv/\yl in {3000/3k,6000/6k,9000/9k,12000/12k}{
  \pgfmathsetmacro{\yp}{\yv*\ysc}
  \draw(0,\yp)--(-0.1,\yp) node[left]{\yl};
}
% x ticks
\foreach \xv/\xl in {0/0,1000/1k,2000/2k}{
  \pgfmathsetmacro{\xp}{\xv*\xsc}
  \draw(\xp,0)--(\xp,-0.1) node[below]{\xl};
}
% KL_v no-LoRA: 10779->4103
\draw[paperB, very thick]
  (0,{10779*\ysc}) -- (4,{4103*\ysc});
\fill[paperB](0,{10779*\ysc}) circle(2pt);
\fill[paperB](4,{4103*\ysc}) circle(2pt);
\node[paperB, anchor=west, font=\tiny] at (4.05,{4103*\ysc}) {$\mathrm{KL}_v$ no LoRA};
% KL_v w-LoRA: 10665->4305
\draw[paperC, very thick]
  (0,{10665*\ysc}) -- (4,{4305*\ysc});
\fill[paperC](0,{10665*\ysc}) circle(2pt);
\fill[paperC](4,{4305*\ysc}) circle(2pt);
\node[paperC, anchor=west, font=\tiny] at (4.05,{4305*\ysc}) {$\mathrm{KL}_v$ w/ LoRA};
% KL_a: 494->486 (flat)
\draw[paperA, very thick, dashed]
  (0,{494*\ysc}) -- (4,{486*\ysc});
\fill[paperA](0,{494*\ysc}) circle(2pt);
\fill[paperA](4,{486*\ysc}) circle(2pt);
\node[paperA, anchor=west, font=\tiny] at (4.05,{490*\ysc}) {$\mathrm{KL}_a$ (frozen)};
\end{tikzpicture}\\[2pt]
{\tiny $\beta$ halves the video rate; the audio rate never moves.}
\end{column}
