...
\begin{block}{Diagnosed \emph{why} the model ignores audio --- it's the benchmark (``Asset A'')}
  Causal per-modality ablation (E1) on MUSIC-AVQA: only $\sim$0.5\,\% of
  questions are genuinely audio-visual; $\sim$46.5\,\% are answerable from the
  language prior alone. The benchmark structurally \textbf{cannot} test an AV
  bottleneck.
\end{block}

\begin{block}{Found the best architecture: T2 = frozen LLM + VIB only (no LoRA)}
...
\begin{block}{Tested fail-safety on AVHBench --- and it regressed}
  T2 became \textbf{more credulous} (Video-driven Audio Hallucination
...
\begin{frame}{Asset A --- MUSIC-AVQA cannot test an AV bottleneck}

\begin{columns}[T]
\begin{column}{0.50\textwidth}

\begin{block}{E1: causal per-modality ablation}
{\footnotesize
For each test item, replace the spliced tokens and measure the change:
\begin{itemize}\setlength\itemsep{1pt}
  \item \texttt{identity} --- reference
  \item \texttt{video\_only} --- drop audio
  \item \texttt{audio\_only} --- drop video
  \item \texttt{zero\,/\,mean\,/\,shuffle} --- destroy both
\end{itemize}
If accuracy barely moves when a modality is removed, the model never needed it.}
\end{block}
...
\midrule
Prior-driven questions      & $\sim$46.5\,\% \\
\rowcolor{paperD!12}
Genuinely AV-essential      & $\sim$0.5\,\% \\
Cost of dropping video (\texttt{video\_only}) & $+4.4$\,\% NLL \\
Audio channel rate $\mathrm{KL}_a$ (all runs) & frozen $\sim$477 \\
\bottomrule
\end{tabular}}
\end{block}
...
{\footnotesize
The visual-shortcut behaviour we flagged last week (AVHBench AV-Matching at
50\,\%, audio $-2.7$\,pp) is not a model bug to be ``fixed'' by contrastive
training --- it is the \textbf{rational solution} to a benchmark whose answers
are mostly recoverable without audio.}
\end{block}

\begin{block}{External corroboration}
{\footnotesize
\textbf{Omni-R1} (MIT, on Qwen2.5-Omni) independently finds:
\begin{itemize}\setlength\itemsep{1pt}
  \item text-only fine-tuning \emph{improves} audio-benchmark accuracy;
  \item audio-withheld inference shows gains come from better text reasoning.
\end{itemize}
Same disease, sibling model. Our contribution is the \textbf{causal,
per-channel} version --- and a remedy.}
\end{block}
