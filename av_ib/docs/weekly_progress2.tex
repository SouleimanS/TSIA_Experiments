...
\begin{frame}{Frozen LLM + bottleneck only is the best configuration}
...
\vspace{2pt}
{\footnotesize \textbf{Video-only bottleneck}: only the video VIB trains;
...
\midrule
LoRA + bottleneck ($\beta$=7e-6) & 80\,\% \\
\rowcolor{paperD!12}
\textbf{bottleneck only (no LoRA)}  & \textbf{84\,\%} \\
\bottomrule
\end{tabular}}
\end{block}
