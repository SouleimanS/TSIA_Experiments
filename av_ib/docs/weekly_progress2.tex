...
\begin{tabular}{l l r}
\toprule
\textbf{What we delete} & \textbf{Evidence left} & \textbf{$\Delta$NLL} \\
\midrule
nothing \emph{(reference)} & audio + video & --- \\
\rowcolor{paperD!12}
audio & \textbf{video only} & \textbf{$+4.4$\,\%} \\
video & audio only & TODO \\
both & nothing & TODO \\
\bottomrule
\end{tabular}}
\end{column}
...
{\footnotesize
Dropping video costs almost nothing ($+4.4$\,\%), and only $\sim$\textbf{0.5\,\%}
of questions are genuinely audio-visual. The audio channel stays inert
throughout (rate $\mathrm{KL}_a$ frozen $\sim$477).}
\end{block}
\end{column}
\end{columns}
...
{\footnotesize
MUSIC-AVQA is largely solvable from the language prior, so it \textbf{cannot
validate an audio-visual bottleneck} --- which is why we pivot the evaluation
(next slides).}
\end{block}
