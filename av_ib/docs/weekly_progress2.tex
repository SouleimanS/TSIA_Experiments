...
{\footnotesize
At \emph{matched} compression ($\mathrm{KL}_v\approx4.7$k), bottleneck-only on a
frozen LLM (84\,\%) beats LoRA$+$bottleneck (80\,\%) --- so freezing the LLM is
the best \emph{way to add} a bottleneck. It costs only $\sim$2\,pp vs.\ the
uncompressed baseline for \textbf{2.6$\times$} video compression.\\[3pt]
Audio rate is \emph{stuck} ($\mathrm{KL}_a$ $494\to486$ in training): the audio
bottleneck has no task signal to learn, so we leave audio untouched.}
\end{block}
...
\vspace{2pt}
\begin{block}{Why keep studying the bottleneck if plain LoRA scores higher?}
{\footnotesize
Accuracy on this benchmark is the \emph{wrong} yardstick --- slide~3 showed it
doesn't need audio-visual evidence, so 86\,\% vs 84\,\% measures the prior, not
grounding. The bottleneck's value is a \textbf{controllable information rate}: it
discards $2.6\times$ of the video signal for $\sim$2\,pp, and --- the real goal
--- gives an explicit knob for \textbf{fail-safe behaviour} (abstain when
evidence is corrupted). That is what the next slides test.}
\end{block}
