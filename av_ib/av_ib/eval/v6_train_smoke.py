"""v6 sink-aware forward_train smoke test for AVModelV6.
...
def main(args):
    print("=" * 60)
    print("v6 sink-aware smoke test for AVModelV6")
    print("=" * 60)

    print("\n[1/8] Constructing AVModelV5...")
    t0 = time.time()
    try:
        from av_ib.model.av_model_v6 import AVModelV6
        model = AVModelV6(use_lora=True)
        model.train()
    ...
    print("\n[2/8] Loading one MUSIC-AVQA record...")
    try:
    ...
    print("\n[4/8] Sink mask sanity check...")
    # v6 should have computed a sink mask during the forward pass. We can't
    # access it directly via the public API, so re-run sink classification
    # manually on the encoder features to confirm the mask is non-trivial.
    # First, get the video encoder output for our sample record.
    from av_ib.model.av_model_v6 import _classify_sinks, DSINK_DIMS, SINK_TAU
    print(f"  Sink config: dims={DSINK_DIMS}, tau={SINK_TAU}")
    print(f"  Cannot directly inspect mask without re-running forward,")
    print(f"  but Phase A validated this proxy produces ~36% sink rate at tau=18 (video).")
    # Sanity assertion via a synthetic check on a known-massive activation pattern:
    fake = torch.zeros(1, 5, model.D_MODEL)
    fake[0, 0, DSINK_DIMS[0]] = 100.0  # very massive on dim 985
    fake[0, 2, DSINK_DIMS[1]] = 100.0  # very massive on dim 1992
    sink_mask = _classify_sinks(fake, DSINK_DIMS, SINK_TAU)
    print(f"  Sanity: synthetic input with massive activation on Dsink dims")
    print(f"    -> sink_mask = {sink_mask[0].tolist()}")
    if not (sink_mask[0, 0].item() and sink_mask[0, 2].item()):
        print(f"  FAIL: massive activations on Dsink should produce sink=True")
        return 4
    if sink_mask[0, 1].item() or sink_mask[0, 3].item() or sink_mask[0, 4].item():
        print(f"  FAIL: zero activations on Dsink should produce sink=False")
        return 4
    print(f"  Sink classifier behaves correctly on synthetic input.")

    print("\n[5/8] Six-term loss inspection...")
    ...
    # v6: kl_v / kl_j are already weighted by beta_sink_ratio inside the model.
    # The smoke uses beta=1e-3 like v5; trainer would also multiply, but here we
    # follow the v5 smoke convention to compare loss magnitudes directly.
    ...
    print("\n[8/8] Loss magnitude sanity check (cosmetic)...")
    ...
    print("\n" + "=" * 60)
    print("v6 SINK-AWARE SMOKE PASSED — backprop reaches all added modules.")
